    ...
    function DisplayTime () {
        ...
        tdt = new Date(utc.getTime() + ((LeapSecondCount(utc) + 32.184) * 1000));
        gps = new Date(tai.getTime() - (19 * 1000));
        loran = new Date(tai.getTime() - (10 * 1000));


        document.clock.ans2a.value = FormatIso8601(utc);
        document.clock.ans2b.value = WeekDayName(utc);
        document.clock.ans2c.value = "day " + Fixed3(GetDayOfYear(utc));
        document.clock.ans2d.value = "MJD " + GetMjd1(tai) +
           "." + Fixed5(Math.floor(GetElapsed(utc) / 86400 * 1e5));

        document.clock.ans5a.value = FormatIso8601(tai);
        document.clock.ans5b.value = WeekDayName(tai);
        document.clock.ans5c.value = "day " + Fixed3(GetDayOfYear(tai));
        document.clock.ans5d.value =
            LeapSecondCount(utc) + " leap seconds";
            // "next leap " + NextLeapSecond();
